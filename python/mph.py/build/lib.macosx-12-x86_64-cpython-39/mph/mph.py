...
def compute_spatiotemporal_landscapes_sparse(points: np.ndarray, max_metric_value: float, hom_dim: int) -> SparseLandscape:
    ret = computeSpatiotemporalLandscapesSparse(np.ascontiguousarray(points, dtype=np.float64), mex_metric_value, hom_dim)
    return SparseLandscape(ret)
